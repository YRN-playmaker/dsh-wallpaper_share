...
<div align="center">
  ...
  <a href="https://github.com/YRN-playmaker/dsh-wallpaper_share/releases"><img alt="插件版本 v26.8.31-rc（候选版）" src="https://img.shields.io/badge/v26.8.31--rc-4d6bfe" /></a><br /><br />
...
#   从 npm 安装
dsh plugin --profile web add ./dsh-wallpaper_share-26.8.31-rc.tgz
#   本地 tarball 安装（26.8.31-rc 候选版）
...
> 适用版本：插件 `v26.8.31-rc` / Harness `0.1.2-alpha.2`。
...
### 内部逻辑缺陷（待修复，生产环境恢复后处理）

- **路径穿越漏洞（安全）**：`/we-sync/wallpaper/` 和 sourceServer 中 `normalize()` 不做 `..` 解析，`startsWith` 检查可被 `../` 绕过，攻击者可读取任意文件。
- **`poll()` 轮询永久锁死**：`!res.ok` 时提前返回未重置 `polling=false`，任何非 200 响应使壁纸同步永久停止（需刷新页面恢复）。
- **DWP 渲染循环遇异常停摆**：运行时 GL 异常后 `raf=0; return`，无降级 Canvas2D 重试，背景永久冻结。
- **`decode()` RAW 帧无长度校验**：payload 不足 `w*h*4` 时同步抛 RangeError，丢帧刷屏。
...
#   install from npm
dsh plugin --profile web add ./dsh-wallpaper_share-26.8.31-rc.tgz
#   install from a local tarball (26.8.31-rc release candidate)
...
> Applies to plugin `v26.8.31-rc` / Harness `0.1.2-alpha.2`.
...
### Internal logic defects (pending fix after the production environment is back)

- **Path traversal (security)**: `normalize()` in `/we-sync/wallpaper/` and the source server does not resolve `..`, so the `startsWith` guard can be bypassed with `../` to read arbitrary files.
- **`poll()` permanently deadlocks**: `if (!res.ok) return;` skips resetting `polling = false`, so any non-200 response stops wallpaper sync forever (needs a page refresh).
- **DWP render loop freezes on runtime errors**: a GL exception sets `raf = 0; return` with no Canvas2D fallback retry, freezing the background permanently.
- **`decode()` RAW frames lack length validation**: a short payload throws a synchronous `RangeError`, dropping frames and spamming the console.
