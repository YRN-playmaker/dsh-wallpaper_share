// 渲染 Miku 网格的多种 cropoffset 语义变体，输出对比图供用户目视选择
import { readFileSync, writeFileSync } from 'node:fs'
import zlib from 'node:zlib'
import { join } from 'node:path'
import { fileURLToPath, pathToFileURL } from 'node:url'

const here = fileURLToPath(import.meta.url).replace(/\\/g, '/')
const root = here.slice(0, here.lastIndexOf('/_dev/'))
const { parsePuppetMdl } = await import(pathToFileURL(join(root, 'src/scene/ScenePuppet.ts')).href)
const { decodeTex } = await import(pathToFileURL(join(root, 'src/scene/SceneTex.ts')).href)

function parsePkg(buf) {
  let pos = 16
  const entries = []
  while (pos + 8 <= buf.length) {
    const nameLen = buf.readInt32LE(pos); pos += 4
    if (nameLen <= 0 || nameLen > 1024 || pos + nameLen + 8 > buf.length) break
    const name = buf.subarray(pos, pos + nameLen).toString('utf8'); pos += nameLen
    const offset = buf.readInt32LE(pos); pos += 4
    const size = buf.readInt32LE(pos); pos += 4
    if (offset < 0 || size < 0 || offset + size > buf.length) break
    entries.push({ name, offset, size })
  }
  const dataStart = pos
  return (n) => { const e = entries.find((x) => x.name === n); if (!e) return null; return buf.subarray(dataStart + e.offset, dataStart + e.offset + e.size) }
}

function decodePNG(buf) {
  let pos = 8, w = 0, h = 0, colorType = 0
  const idat = []
  while (pos + 8 <= buf.length) {
    const len = buf.readUInt32BE(pos)
    const type = buf.toString('ascii', pos + 4, pos + 8)
    const data = buf.subarray(pos + 8, pos + 8 + len)
    if (type === 'IHDR') { w = data.readUInt32BE(0); h = data.readUInt32BE(4); colorType = data[9] }
    else if (type === 'IDAT') idat.push(data)
    pos += 12 + len
  }
  const raw = zlib.inflateSync(Buffer.concat(idat))
  const channels = colorType === 6 ? 4 : colorType === 2 ? 3 : colorType === 4 ? 2 : 1
  const stride = w * channels
  const out = new Uint8Array(w * h * 4)
  const prev = new Uint8Array(stride)
  for (let y = 0; y < h; y++) {
    const filter = raw[y * (stride + 1)]
    const line = raw.subarray(y * (stride + 1) + 1, (y + 1) * (stride + 1))
    const cur = new Uint8Array(stride)
    for (let x = 0; x < stride; x++) {
      const a = x >= channels ? cur[x - channels] : 0
      const b = prev[x]
      const c = x >= channels ? prev[x - channels] : 0
      let v = line[x]
      if (filter === 1) v = (v + a) & 0xff
      else if (filter === 2) v = (v + b) & 0xff
      else if (filter === 3) v = (v + ((a + b) >> 1)) & 0xff
      else if (filter === 4) { const p = a + b - c; const pa = Math.abs(p - a), pb = Math.abs(p - b), pc = Math.abs(p - c); v = (v + (pa <= pb && pa <= pc ? a : pb <= pc ? b : c)) & 0xff }
      cur[x] = v
    }
    for (let x = 0; x < w; x++) {
      const o = (y * w + x) * 4
      if (channels === 4) { out[o] = cur[x * 4]; out[o + 1] = cur[x * 4 + 1]; out[o + 2] = cur[x * 4 + 2]; out[o + 3] = cur[x * 4 + 3] }
      else if (channels === 3) { out[o] = cur[x * 3]; out[o + 1] = cur[x * 3 + 1]; out[o + 2] = cur[x * 3 + 2]; out[o + 3] = 255 }
      else if (channels === 2) { const g = cur[x * 2], al = cur[x * 2 + 1]; out[o] = g; out[o + 1] = g; out[o + 2] = g; out[o + 3] = al }
      else { const g = cur[x]; out[o] = g; out[o + 1] = g; out[o + 2] = g; out[o + 3] = 255 }
    }
    prev.set(cur)
  }
  return { w, h, rgba: out }
}

function writePNG(path, w, h, rgba) {
  const crcTable = []
  for (let n = 0; n < 256; n++) { let c = n; for (let k = 0; k < 8; k++) c = c & 1 ? 0xedb88320 ^ (c >>> 1) : c >>> 1; crcTable[n] = c >>> 0 }
  const crc32 = (buf) => { let c = 0xffffffff; for (const b of buf) c = crcTable[(c ^ b) & 0xff] ^ (c >>> 8); return (c ^ 0xffffffff) >>> 0 }
  const chunk = (type, data) => {
    const len = Buffer.alloc(4); len.writeUInt32BE(data.length)
    const td = Buffer.concat([Buffer.from(type, 'ascii'), data])
    const crc = Buffer.alloc(4); crc.writeUInt32BE(crc32(td))
    return Buffer.concat([len, td, crc])
  }
  const raw = Buffer.alloc(h * (1 + w * 4))
  for (let y = 0; y < h; y++) {
    raw[y * (1 + w * 4)] = 0
    for (let x = 0; x < w; x++) {
      const o = y * (1 + w * 4) + 1 + x * 4
      const s = (y * w + x) * 4
      raw[o] = rgba[s]; raw[o + 1] = rgba[s + 1]; raw[o + 2] = rgba[s + 2]; raw[o + 3] = rgba[s + 3]
    }
  }
  const ihdr = Buffer.alloc(13)
  ihdr.writeUInt32BE(w, 0); ihdr.writeUInt32BE(h, 4)
  ihdr[8] = 8; ihdr[9] = 6
  const png = Buffer.concat([Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]), chunk('IHDR', ihdr), chunk('IDAT', zlib.deflateSync(raw, { level: 6 })), chunk('IEND', Buffer.alloc(0))])
  writeFileSync(path, png)
}

const read = parsePkg(readFileSync('D:/SteamLibrary/steamapps/workshop/content/431960/3409595232/scene.pkg'))
const tex = decodeTex(read('materials/导出初音.tex'))
const png = decodePNG(tex.mip0.data)
const { rgba, w: tw, h: th } = png
const pm = parsePuppetMdl(read('models/导出初音_puppet.mdl'))
const mesh = pm.mesh
// cropoffset（模型 JSON）
const cropX = -220
const cropY = 434.5
const layerW = 4862
const layerH = 3288

// 网格范围
let mnx = Infinity, mny = Infinity, mxx = -Infinity, mxy = -Infinity
for (const v of mesh.vertices) {
  if (v.pos[0] < mnx) mnx = v.pos[0]
  if (v.pos[0] > mxx) mxx = v.pos[0]
  if (v.pos[1] < mny) mny = v.pos[1]
  if (v.pos[1] > mxy) mxy = v.pos[1]
}
const meshW = mxx - mnx
const meshH = mxy - mny
console.log('网格: x[' + mnx.toFixed(0) + ',' + mxx.toFixed(0) + '] y[' + mny.toFixed(0) + ',' + mxy.toFixed(0) + ']，' + meshW.toFixed(0) + 'x' + meshH.toFixed(0))
console.log('图层: ' + layerW + 'x' + layerH + ' cropoffset=(' + cropX + ',' + cropY + ')')

// 渲染到 480 宽画布（图层空间），各种变体
const S = 480 / layerW
const W = 480
const H = Math.round(layerH * S)

function renderVariant(label, transform) {
  // transform: (x, y) => [px, py] 模型空间 → 图层像素（y-down）
  const img = new Uint8Array(W * H * 4)
  for (let i = 0; i + 2 < mesh.indices.length; i += 3) {
    const a = mesh.vertices[mesh.indices[i]], b = mesh.vertices[mesh.indices[i + 1]], cc = mesh.vertices[mesh.indices[i + 2]]
    const u0 = a.uv[0] * tw, v0 = a.uv[1] * th
    const u1 = b.uv[0] * tw, v1 = b.uv[1] * th
    const u2 = cc.uv[0] * tw, v2 = cc.uv[1] * th
    const [x0, y0] = transform(a.pos[0], a.pos[1])
    const [x1, y1] = transform(b.pos[0], b.pos[1])
    const [x2, y2] = transform(cc.pos[0], cc.pos[1])
    const area = (x1 - x0) * (y2 - y0) - (y1 - y0) * (x2 - x0)
    if (Math.abs(area) < 1e-3) continue
    const minX = Math.max(0, Math.floor(Math.min(x0, x1, x2))), maxX = Math.min(W - 1, Math.ceil(Math.max(x0, x1, x2)))
    const minY = Math.max(0, Math.floor(Math.min(y0, y1, y2))), maxY = Math.min(H - 1, Math.ceil(Math.max(y0, y1, y2)))
    for (let y = minY; y <= maxY; y++) for (let x = minX; x <= maxX; x++) {
      const Px = x + 0.5, Py = y + 0.5
      const w0 = ((x1 - Px) * (y2 - Py) - (y1 - Py) * (x2 - Px)) / area
      const w1 = ((x2 - Px) * (y0 - Py) - (y2 - Py) * (x0 - Px)) / area
      const w2 = 1 - w0 - w1
      if (w0 < -1e-6 || w1 < -1e-6 || w2 < -1e-6) continue
      const u = Math.min(tw - 1, Math.max(0, Math.floor(w0 * u0 + w1 * u1 + w2 * u2)))
      const v = Math.min(th - 1, Math.max(0, Math.floor(w0 * v0 + w1 * v1 + w2 * v2)))
      const so = (v * tw + u) * 4
      const o = (y * W + x) * 4
      img[o] = rgba[so]; img[o + 1] = rgba[so + 1]; img[o + 2] = rgba[so + 2]; img[o + 3] = 255
    }
  }
  writePNG('_dev/out-' + label + '.png', W, H, img)
  // 覆盖统计
  let covered = 0
  for (let i = 0; i < W * H; i++) if (img[i * 4] !== 0 || img[i * 4 + 1] !== 0 || img[i * 4 + 2] !== 0) covered++
  console.log(label + ': 覆盖 ' + covered + 'px (' + (covered / (W * H) * 100).toFixed(1) + '%)')
}

// V0 当前：模型原点 → 图层中心（size/2），无缩放
renderVariant('miku-v0', (x, y) => [(layerW / 2 + x) * S, (layerH / 2 - y) * S])
// V1 非等比缩放铺满：网格 → 图层
renderVariant('miku-v1', (x, y) => [((x - mnx) / meshW) * layerW * S, ((mxy - y) / meshH) * layerH * S])
// V2 等比缩放 min 居中（y 满）
const scaleMin = Math.min(layerW / meshW, layerH / meshH)
const offX2 = (layerW - meshW * scaleMin) / 2
const offY2 = (layerH - meshH * scaleMin) / 2
renderVariant('miku-v2', (x, y) => [((x - mnx) * scaleMin + offX2) * S, ((mxy - y) * scaleMin + offY2) * S])
// V3 等比缩放 max 居中（x 满）
const scaleMax = Math.max(layerW / meshW, layerH / meshH)
const offX3 = (layerW - meshW * scaleMax) / 2
const offY3 = (layerH - meshH * scaleMax) / 2
renderVariant('miku-v3', (x, y) => [((x - mnx) * scaleMax + offX3) * S, ((mxy - y) * scaleMax + offY3) * S])
// V4 cropoffset 平移（模型空间顶点 + cropoffset）
renderVariant('miku-v4', (x, y) => [(layerW / 2 + x + cropX) * S, (layerH / 2 - y + cropY) * S])
// V5 网格内容中心 → cropoffset 位置（网格中心移到 cropoffset，y-up）
const cx0 = (mnx + mxx) / 2
const cy0 = (mny + mxy) / 2
renderVariant('miku-v5', (x, y) => [(layerW / 2 + (x - cx0) + cropX) * S, (layerH / 2 - (y - cy0) + cropY) * S])
// V6 网格中心移到图层中心（消除网格非对称）
renderVariant('miku-v6', (x, y) => [(layerW / 2 + (x - cx0)) * S, (layerH / 2 - (y - cy0)) * S])
console.log('done')
