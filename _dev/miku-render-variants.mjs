// 渲染 3 版 Miku 网格（不同 UV/翻转假设）供用户对比 WE 截图
...
function renderVariant(flipV, flipY) {
  ...
  for (let t = 0; t + 2 < mesh.indices.length; t += 3) {
    ...
    const ax = toX(a.pos[0]), ay = flipY ? H - 10 - (a.pos[1] - mny) * sx : toY(a.pos[1])
    const bx = toX(b.pos[0]), by = flipY ? H - 10 - (b.pos[1] - mny) * sx : toY(b.pos[1])
    const cx = toX(c.pos[0]), cy = flipY ? H - 10 - (c.pos[1] - mny) * sx : toY(c.pos[1])
...
writePNG('_dev/out-miku-v1.png', W, H, renderVariant(true, false))
writePNG('_dev/out-miku-v2.png', W, H, renderVariant(false, false))
writePNG('_dev/out-miku-v3.png', W, H, renderVariant(true, true))
console.log('rendered: v1 (v翻), v2 (v不翻), v3 (v翻+y不翻)')
