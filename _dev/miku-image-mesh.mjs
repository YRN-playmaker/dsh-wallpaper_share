// 渲染"立绘 + 网格变形"合成 vs 当前"仅网格区域"，对比官方
// A: 当前实现（仅网格区域） B: 立绘为底 + 网格区域重绘 C: 立绘为底 + 网格区域重绘(cropoffset 偏移)
...
const mesh = pm.mesh
const layerW = 4862, layerH = 3288
const S = 480 / layerW
const W = 480
const H = Math.round(layerH * S)

// 缩放画布（立绘 → 480 宽）
function scaleImg(src, sw, sh, dw, dh) {
...
const base = scaleImg(rgba, tw, th, W, H)

// 网格渲染：以 (0,0) 为图层中心的画布上画网格三角形（clip + bary 采样）
function renderMesh(canvas, ox, oy, tx, ty) {
  // ox/oy: 图层中心在画布中的像素位置；tx/ty: 额外平移（cropoffset，画布像素）
  for (let i = 0; i + 2 < mesh.indices.length; i += 3) {
    ...
    // 模型空间 → 画布像素（y-up → y-down）
    const x0 = ox + a.pos[0] * S + tx, y0 = oy - a.pos[1] * S + ty
    const x1 = ox + b.pos[0] * S + tx, y1 = oy - b.pos[1] * S + ty
    const x2 = ox + cc.pos[0] * S + tx, y2 = oy - cc.pos[1] * S + ty
    ...
    for (let y = minY; y <= maxY; y++) for (let x = minX; x <= maxX; x++) {
      ...
      const so = (v * tw + u) * 4
      const o = (y * W + x) * 4
      canvas[o] = rgba[so]; canvas[o + 1] = rgba[so + 1]; canvas[o + 2] = rgba[so + 2]; canvas[o + 3] = 255
    }
  }
}
...
// A: 当前实现（仅网格区域，透明背景）
{
  ...
  renderMesh(img, W / 2, H / 2, 0, 0)
  writePNG('_dev/out-miku-A-grid-only.png', W, H, img)
  let c = 0
  for (let i = 0; i < W * H; i++) if (img[i * 4] !== 0 || img[i * 4 + 1] !== 0) c++
  console.log('A 网格仅: ' + c + 'px')
}
// B: 立绘为底 + 网格重绘
{
  const img = new Uint8Array(base)
  renderMesh(img, W / 2, H / 2, 0, 0)
  writePNG('_dev/out-miku-B-image-plus-mesh.png', W, H, img)
}
// C: 立绘为底 + 网格重绘（cropoffset 平移：-220*S, +434.5*S 画布像素）
{
  const img = new Uint8Array(base)
  renderMesh(img, W / 2, H / 2, -220 * S, 434.5 * S)
  writePNG('_dev/out-miku-C-crop-offset.png', W, H, img)
}
// D: 立绘原图（无网格）参考
{
  writePNG('_dev/out-miku-D-image-only.png', W, H, base)
}
console.log('done')
