...
const binds = []
let q = mdls + 17
for (let i = 0; i < 5 && q + 77 <= b.length; i++) {
...
// 瞳孔顶点（bone1 权重最高）
const pupilVerts = verts.map((v,i)=>({i,v})).filter(o => { const bi = o.v.boneIdx.indexOf(1); return bi >= 0 && o.v.weights[bi] > 0.8 })
console.log('\n瞳孔顶点数: '+pupilVerts.length)
const f0 = skinFrame(0), f4 = skinFrame(4)
let moved = 0
for (const {i} of pupilVerts) if (Math.hypot(f4[i][0]-f0[i][0], f4[i][1]-f0[i][1]) > 1) moved++
console.log('瞳孔顶点位移>1px: '+moved+'/'+pupilVerts.length)
const cen = (pts) => pts.reduce((a,p)=>[a[0]+p[0],a[1]+p[1]],[0,0]).map(x=>x/pts.length)
const c0 = cen(pupilVerts.map(o=>f0[o.i])), c4 = cen(pupilVerts.map(o=>f4[o.i]))
console.log('瞳孔质心: f0=('+c0[0].toFixed(2)+','+c0[1].toFixed(2)+') f4=('+c4[0].toFixed(2)+','+c4[1].toFixed(2)+')')
let d0=0,d4=0
for (const {i} of pupilVerts) { d0 += Math.hypot(f0[i][0]-c0[0],f0[i][1]-c0[1]); d4 += Math.hypot(f4[i][0]-c4[0],f4[i][1]-c4[1]) }
console.log('平均距离到质心: f0='+(d0/pupilVerts.length).toFixed(2)+' f4='+(d4/pupilVerts.length).toFixed(2))
// 下眼睑
const lidVerts = verts.map((v,i)=>({i,v})).filter(o => { const bi = o.v.boneIdx.indexOf(3); return bi >= 0 && o.v.weights[bi] > 0.8 })
console.log('\n下眼睑顶点数: '+lidVerts.length)
moved = 0
for (const {i} of lidVerts) if (Math.hypot(f4[i][0]-f0[i][0], f4[i][1]-f0[i][1]) > 1) moved++
console.log('下眼睑位移>1px: '+moved+'/'+lidVerts.length)
// 总体包围盒
...
console.log('\n包围盒 f0: '+bb(f0).map(x=>x.toFixed(1)).join(' '))
console.log('包围盒 f4: '+bb(f4).map(x=>x.toFixed(1)).join(' '))
