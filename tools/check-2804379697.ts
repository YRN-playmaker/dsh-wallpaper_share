...
interface Pkg { entries: Entry[]; dataStart: number; read(name: string): Uint8Array | null }
function readPkg(path: string): Pkg {
  ...
  const dataStart = pos
  const read = (n: string): Uint8Array | null => { const e = entries.find((x) => x.name === n); return e !== undefined ? buf.subarray(dataStart + e.offset, dataStart + e.offset + e.size) : null }
  return { entries, dataStart, read }
}
...
const matTexMap = new Map<string, string[]>()
const matSprite = new Map<string, boolean>()
...
while (idx < objBlock.length && count < 60) {
  ...
  if (idM !== null) {
    console.log('  id=' + idM[1] + ' name="' + (nameM !== null ? nameM[1] : '') + '"' + (imageM !== null ? ' img=' + imageM[1] : '') + ' effs=' + effCnt)
  }
  idx = end + 1; count++
}
console.log('\n=== model → 材质 → 纹理 ===')
const modelMatMap = new Map<string, string | null>()
for (const e of pkg.entries) {
...
for (const mr of [...new Set([...sceneText.matchAll(/"image"\s*:\s*"([^"]+)"/g)].map((m) => m[1]))]) {
  ...
  const sp = matSprite.get(mat) ?? false
  for (const tn of texs) {
    ...
    if (b !== undefined && b !== null) {
      try { const tex = decodeTex(b); frames = tex !== null && tex.frames !== null ? tex.frames.length : 0; kind = tex?.mip0?.kind ?? '' } catch (e) { kind = 'ERR:' + String(e) }
    }
    console.log('  ' + mr + ' → ' + mat + ' → ' + tname + ' spritesheet=' + sp + (frames > 0 ? ' [TEXS ' + frames + '帧 kind=' + kind + ']' : ' kind=' + kind))
  }
}
