...
var SceneAdapter = class {
	...
	checkHealth() {
		...
		if (proc.lastFrameAt > 0 && Date.now() - proc.lastFrameAt > STALL_MS) {
			this.log("[SceneRenderer] No frame for 4000ms — restarting renderer");
			proc.kill();
		}
	}
	getCapabilities() {
...
function resolveParticleSystem(pkg, ref, obj) {
	try {
		...
		const rate = em.rate !== void 0 ? numOr(em.rate, 1) : override.rate !== void 0 ? numOr(override.rate, 1) : Math.max(1, Math.round(maxcount / 15));
		const emitter = {
		...
		for (const init of inits) {
			...
			if (name === "lifetimerandom") initializers.lifetime = [mn, mx];
				...
				initializers.colorMin = parseVec3(init.min, [
					1,
					1,
					1
				]);
				initializers.colorMax = parseVec3(init.max, [
					1,
					1,
					1
				]);
			} else if (name === "turbulentvelocityrandom") initializers.turbulentVelocity = {
				offset: numOr(init.offset, -.5),
				scale: numOr(init.scale, .1)
			};
		...
		if (Array.isArray(preset.children)) {
			for (const c of preset.children) if (c !== null && typeof c === "object" && typeof c.name === "string") {
				const child = resolveParticleSystem(pkg, c.name, obj);
		...
		if (typeof override.alpha === "number") {
			initializers.alphaMin = override.alpha;
			initializers.alphaMax = override.alpha;
		}
		...
		if (typeof override.count === "number") {}
		return {
			particleRef: ref,
			materialRef: matRef,
			blending,
	...
	*  name 如 particle/fog/fog1（材质 textures 的相对路径） */
	disposers.push(webServer.register({
		...
		handler(req, res) {
			...
			try {
				const tex = decodeTex(new Uint8Array(readFileSync(state.weDir + "/assets/materials/" + name + ".tex")));
