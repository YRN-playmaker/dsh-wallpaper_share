...
function resolveParticleSystem(pkg, ref, obj) {
	try {
		...
		if (typeof override.alpha === "number") {
			...
			if (initializers.alphaMin !== void 0) {
	...
	*  'auto' → 显式配置了 sceneRendererPath 则 external，否则 browser（浏览器子集渲染器为主路线） */
	...
	disposers.push(webServer.register({
		...
		handler(_req, res) {
			...
			sendJson(res, { apps: appsCache.map((a) => ({
				id: a.id,
				title: a.title,
				file: a.file,
	...
	disposers.push(webServer.register({
		...
		handler(req, res) {
			...
			const workshopDir = resolveWorkshopDir(state.weDir);
			const app = scanApps(state.weDir, workshopDir).find((a) => a.id === id);
	...
	disposers.push(webServer.register({
		...
		handler(req, res) {
			...
			execFile("explorer.exe", ["/select," + target], { windowsHide: true }, () => {
				execFile("explorer.exe", [app.dir], { windowsHide: true }, () => {});
			});
			sendJson(res, {
				opened: true,
				dir: app.dir
