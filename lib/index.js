...
import { appendFileSync, closeSync, cpSync, createReadStream, existsSync, mkdirSync, openSync, readFileSync, readdirSync, realpathSync, renameSync, rmSync, statSync, writeFileSync, writeSync } from "node:fs";
...
//#region src/editor/installer.ts
const EDITOR_VERSION = "0.2.0";
const EDITOR_URL = "https://github.com/YRN-playmaker/dsh-wallpaper_edit/releases/download/v0.2.0/editor.html";
const EDITOR_SHA256 = "fdc7e4a97ea4b65ddb8531e91a72e28846b7737722404ec208c79695c5c9a432";
const MAX_BYTES = 8388608;
var EditorInstaller = class {
	pending = null;
	directory;
	source;
	download;
	constructor(directory, source = {
		url: EDITOR_URL,
	...
	get file() {
		return join(this.directory, `editor-${EDITOR_VERSION}.html`);
	}
	valid(bytes) {
		return createHash("sha256").update(bytes).digest("hex") === this.source.sha256;
	}
	async content() {
		try {
			const b = await readFile(this.file);
			return this.valid(b) ? b : null;
		} catch (e) {
	...
	async status() {
		return {
			installed: await this.content() !== null,
			version: EDITOR_VERSION,
			installing: this.pending !== null
		};
	}
	async install() {
	...
	async acquire() {
		...
		const response = await this.download(this.source.url, { signal: AbortSignal.timeout(12e4) });
		...
		try {
			for (;;) {
				...
				if (size > MAX_BYTES) throw new Error("编辑器下载超出大小限制。");
				chunks.push(value);
			}
		} finally {
...
//#endregion
//#region src/editor/routes.ts
function createEditorRoutes(installer) {
	...
	return [{
		...
		async handler(req, res) {
			try {
				...
				try {
					sameOrigin = !!h.origin && new URL(h.origin).host === h.host;
				} catch {}
				...
				return json(res, 500, { error: e instanceof Error ? e.message : "编辑器获取失败，请重试。" });
			}
		}
	}, {
		kind: "exact",
		path: "/we-sync/editor/app",
		async handler(req, res) {
			if (req.method !== "GET") {
				res.statusCode = 405;
				res.end();
	...
	try {
		...
		if (mdla6 >= 0) try {
			...
			for (let a = 0; a < animCount && p + 12 <= len; a++) {
				...
				if (!(bytes[p] === 240 && bytes[p + 1] === 65)) break;
				p += 2;
				...
				if (boneCount < 1 || boneCount > 512 || frameCount < 1 || frameCount > 8192 || !Number.isInteger(segRows) || segRows < frameCount || segRows > frameCount + 8) break;
				const dataStart = p;
				const declared = boneCount * segRows * 9;
...
function crc32$2(buf) {
	let c = 4294967295;
	for (let i = 0; i < buf.length; i++) c = CRC_TABLE$3[(c ^ buf[i]) & 255] ^ c >>> 8;
...
function createMarketRoutes(deps) {
	...
	const ttlMs = deps.catalogTtlMs ?? 3e5;
	...
	* 又按目录里的 1.0.0 "更新"了回去。
	*/
	const ensureCatalog = async (refresh) => {
		if (!refresh && cache !== null && now() - fetchedAt < ttlMs) return cache;
		const url = refresh ? deps.catalogUrl + (deps.catalogUrl.includes("?") ? "&" : "?") + "t=" + String(now()) : deps.catalogUrl;
	...
	return [
		{
			...
			handler: async (req, res) => {
				try {
					json$3(res, 200, await ensureCatalog(query$2(req).get("refresh") === "1"));
				} catch (e) {
					json$3(res, 502, { error: String(e.message ?? e) });
				}
			}
		},
		{
			...
			path: base + "/installed",
			handler: (_req, res) => {
				json$3(res, 200, { installed: deps.market.installed() });
			}
		},
		{
			kind: "exact",
			...
			handler: async (req, res) => {
				const q = query$2(req);
				const id = q.get("id");
				if (!id) return json$3(res, 400, { error: "缺 id" });
				let entry;
				try {
					entry = (await ensureCatalog(false)).entries.find((e) => e.id === id);
				} catch (e) {
					return json$3(res, 502, { error: String(e.message ?? e) });
				}
				if (!entry) return json$3(res, 404, { error: `catalog 无此 id: ${id}` });
				const rec = deps.market.installed().find((r) => r.id === id);
				const next = entry.dwp.package.version;
				if (rec !== void 0 && q.get("force") !== "1" && compareVersion(next, rec.version) < 0) return json$3(res, 409, {
				...
				try {
					...
					if (e instanceof NeedsPurchaseError) return json$3(res, 402, {
						error: "needs-purchase",
						salesUrl: e.salesUrl,
						platform: e.platform
					});
					return json$3(res, 500, { error: String(e.message ?? e) });
				}
			}
		},
		{
		...
		},
		{
			...
			handler: async (req, res) => {
				try {
					json$3(res, 200, { updates: deps.market.updates(await ensureCatalog(query$2(req).get("refresh") === "1")) });
				} catch (e) {
					json$3(res, 502, { error: String(e.message ?? e) });
				}
			}
...
function createDwpServeRoutes(deps) {
	...
	const applied = {
		...
		path: base + "/applied",
		handler: (_q, res) => {
			json$2(res, 200, { applied: deps.apply.get() });
		}
	};
	const apply = {
		kind: "exact",
		...
		handler: (req, res) => {
			...
			const rec = deps.store.get(id);
			if (!rec) return json$2(res, 404, { error: `未安装: ${id}` });
			json$2(res, 200, {
				ok: true,
	...
	const files = {
		...
		handler: (req, res) => {
			...
			if (!m) return json$2(res, 404, { error: "未安装" });
			json$2(res, 200, { files: [...m.keys()] });
		}
	};
	const file = {
		kind: "exact",
		...
		handler: (req, res) => {
			...
			if (!name) return json$2(res, 400, { error: "缺 name" });
			const data = pick(m, name);
			if (!data) return json$2(res, 404, { error: `包内无此文件: ${name}` });
			res.statusCode = 200;
	...
	const jsonEntry = (path, defaultName) => ({
		...
		handler: (req, res) => {
			...
			if (!data) return json$2(res, 404, { error: `包内无 ${want}` });
			res.statusCode = 200;
			res.setHeader("Content-Type", "application/json; charset=utf-8");
			res.setHeader("Cache-Control", "no-store");
...
var WorkspacePulse = class {
	...
	constructor(root, opts = {}) {
		this.root = root;
		this.windowMs = opts.windowMs ?? 9e4;
		this.ttlMs = opts.ttlMs ?? 1500;
		this.maxEntries = opts.maxEntries ?? 2e4;
	...
	changes() {
		...
		for (const c of this.journal) byPath.set(c.rel, c);
	...
	scanOnce(now) {
		...
		for (const [rel, p] of this.prev) if (!cur.has(rel)) this.push({
			name: base(rel),
			rel,
			sign: "-",
			kind: "del",
	...
	walk(dir, depth, cur) {
		...
		for (const name of names) {
			...
			} else if (st.isFile()) cur.set(relative(this.root, full).split(sep).join("/"), {
				mtimeMs: st.mtimeMs,
				size: st.size
			});
		}
...
function crc32$1(bytes) {
	let c = 4294967295;
	for (let i = 0; i < bytes.length; i++) c = CRC_TABLE$2[(c ^ bytes[i]) & 255] ^ c >>> 8;
	return (c ^ 4294967295) >>> 0;
}
function chunk(type, data) {
	...
	view.setUint32(8 + data.length, crc32$1(crcInput), false);
	return out;
}
...
function crc32(buf) {
	let c = 4294967295;
	for (let i = 0; i < buf.length; i++) c = CRC_TABLE$1[(c ^ buf[i]) & 255] ^ c >>> 8;
	return (c ^ 4294967295) >>> 0;
}
...
function buildZipStored(entries) {
	...
	for (const { name, data } of entries) {
		...
		const lfh = /* @__PURE__ */ new Uint8Array(30);
		...
		const cdh = /* @__PURE__ */ new Uint8Array(46);
...
*/
const PULSE_ID = "workspace-pulse";
...
function circleAlpha(dx, dy, r) {
	const d = Math.sqrt(dx * dx + dy * dy);
	return Math.min(1, Math.max(0, r - d + 1.5));
}
function putPx(rgba, w, x, y, r, g, b, a) {
	const i = (y * w + x) * 4;
	rgba[i] = r;
	rgba[i + 1] = g;
	rgba[i + 2] = b;
...
function renderBubblePng(size = 192) {
	const rgba = new Uint8Array(size * size * 4);
	const c = size / 2, R = size / 2 - 2;
	for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
		const dx = x + .5 - c, dy = y + .5 - c;
		...
		const shade = 1 - .35 * Math.max(0, d - .6) / .4;
		r = Math.round(r * shade);
		g = Math.round(g * shade);
		b = Math.round(b * shade);
		const hl = Math.max(0, 1 - Math.sqrt(dx * dx + (dy + R * .45) ** 2) / (R * .75));
...
function renderPreviewPng(w = 640, h = 360) {
	...
	for (const d of [
		...
		for (let y = Math.max(0, Math.floor(cY - d.R - 2)); y < Math.min(h, Math.ceil(cY + d.R + 2)); y++) for (let x = Math.max(0, Math.floor(cX - d.R - 2)); x < Math.min(w, Math.ceil(cX + d.R + 2)); x++) {
			const dx = x + .5 - cX, dy = y + .5 - cY;
			const a = circleAlpha(dx, dy, d.R);
			if (a <= 0) continue;
			const t = 1 - Math.min(1, Math.sqrt(dx * dx + dy * dy) / d.R);
			const i = (y * w + x) * 4;
			rgba[i] = Math.round(23 + 26 * t);
			rgba[i + 1] = Math.round(26 + 31 * t);
			rgba[i + 2] = Math.round(33 + 39 * t);
			rgba[i + 3] = Math.round(a * 255);
		}
		const bx = cX + d.R * .72, by = cY - d.R * .72, bR = d.R * .36;
		for (let y = Math.max(0, Math.floor(by - bR - 2)); y < Math.min(h, Math.ceil(by + bR + 2)); y++) for (let x = Math.max(0, Math.floor(bx - bR - 2)); x < Math.min(w, Math.ceil(bx + bR + 2)); x++) {
			const a = circleAlpha(x + .5 - bx, y + .5 - by, bR);
			if (a <= 0) continue;
			const i = (y * w + x) * 4;
			rgba[i] = d.badge[0];
			rgba[i + 1] = d.badge[1];
...
function buildPulseScene() {
	...
	const fx = [];
	const slotOpacity = (id, i) => {
		fx.push({
			type: "opacity",
			target: id,
			params: { value: `$b${i}_on` }
		});
	};
	for (let i = 1; i <= 3; i++) {
		const b = `b${i}`;
...
function buildPulsePackage(opts) {
	return buildZipStored([
		{
			name: "wallpaper.json",
			data: new TextEncoder().encode(JSON.stringify(buildPulseManifest()))
		},
		{
			name: "scene.json",
			data: new TextEncoder().encode(JSON.stringify(buildPulseScene()))
		},
		{
			name: "assets/bubble.png",
			data: opts?.bubblePng ?? renderBubblePng()
		},
		{
			name: "assets/preview.png",
			data: opts?.previewPng ?? renderPreviewPng()
		}
	]);
}
//#endregion
...
function crc32Update(crc, byte) {
	return (CRC_TABLE[(crc ^ byte) & 255] ^ crc >>> 8) >>> 0;
}
...
*/
function isLauncherInternalDir(name) {
...
*/
function resolveEntryInside(dir, file) {
	...
	try {
		assertInside(normalize(realpathSync.native(dir)), normalize(realpathSync.native(target)));
	} catch (e) {
...
function samePath(a, b) {
	const norm = (p) => {
		let s = normalize(resolve(p));
		try {
			s = normalize(realpathSync.native(p));
		} catch {}
...
var LauncherInstaller = class {
	...
	moveToRoot(next) {
		...
		for (const rec of list) {
			...
			try {
				...
				from = join(oldRoot, rel);
				assertInside(oldRoot, from);
				to = join(this.root, rel);
				assertInside(this.root, to);
			} catch {
	...
	*     造成两个名字指向同一目录）。 */
	slugify(text) {
		const s = text.trim().toLowerCase().replace(/[\u0000-\u001f<>:"/\\|?*]+/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").replace(/[. ]+$/g, "");
	...
	*  最后才 upsert 索引 —— 于是"替换失败"时盘上是旧的、索引也没动，不会出现"记录说装了、文件却没了"。 */
	finalize(opts) {
		...
		if (opts.commitTo !== void 0) assertInside(this.root, opts.commitTo);
		const rec = {
			id: opts.slug,
			title: opts.title,
			slug: opts.slug,
		...
		if (opts.commitTo !== void 0) this.commitStaging(opts.dir, opts.commitTo);
		this.upsert(rec);
		return rec;
	}
...
//#endregion
//#region src/launcher/helper-sync.ts
/**
...
*  取得下载直链，故不再提供百度登录态同步。）
*/
const HELPER_SYNC_URL = "/we-sync/login-sync.user.js";
const HELPER_SYNC_SCRIPT = `// ==UserScript==
...
*/
const MAX_BODY_BYTES$1 = 4194304;
function query(req) {
	return new URL(req.url ?? "/", "http://localhost").searchParams;
}
	...
	return new Promise((resolve, reject) => {
		...
		stream.on("data", (c) => {
			...
			if (size > MAX_BODY_BYTES$1) {
				done(() => reject(/* @__PURE__ */ new Error(`请求体超过上限 (${Math.round(MAX_BODY_BYTES$1 / 1024)} KiB)`)));
				return;
			}
			chunks.push(c);
		});
...
function createLauncherRoutes(deps) {
	...
	const installed = {
		...
		path: base + "/installed",
		handler: (_req, res) => {
			json$1(res, 200, { installed: installer.list() });
		}
	};
	const rootRoute = {
		kind: "exact",
		path: base + "/root",
		handler: async (req, res) => {
			if (req.method === "GET") return json$1(res, 200, { root: installer.root });
			let body;
			try {
				const raw = await readBody$1(req);
				body = JSON.parse(new TextDecoder().decode(raw));
			} catch (e) {
				return json$1(res, 400, { error: `请求体非法: ${e.message ?? e}` });
			}
			const opts = body;
			if (typeof opts.root !== "string" || opts.root.trim() === "") return json$1(res, 400, { error: "缺 root" });
			try {
				...
				return json$1(res, e instanceof LauncherError ? 400 : 500, { error: msg });
			}
		}
	};
	const install = {
		...
		handler: async (req, res) => {
			...
			const opts = body;
			if (typeof opts.url !== "string" || opts.url.trim() === "") return json$1(res, 400, { error: "缺 url" });
			...
			if (typeof opts.previewDataUrl === "string" && opts.previewDataUrl !== "") {
				...
				if (pv === null) return json$1(res, 400, { error: "previewDataUrl 非法（仅支持 data:image/*;base64）" });
			...
			try {
				...
				if (parse139ShareUrl(url) !== null) try {
					...
					if (e instanceof Yun139Error) return json$1(res, 422, {
						error: e.message,
						code: e.code
					});
					throw e;
				...
				if (integrity !== void 0 && !verifyIntegrity(bytes, integrity)) return json$1(res, 400, { error: "完整性校验失败: sha512 不匹配（包被篡改或损坏）" });
				...
				const dir = installer.appDir(slug);
				...
				let files;
				try {
					if (is7z) files = installer.extract7z(bytes, staging, password, z7off);
					else if (isZip) files = installer.unzipToDir(bytes, staging, password);
					else files = [installer.writeFileToDir(bytes, staging, fileName)];
					...
					if (e instanceof CryptZipError) return json$1(res, 422, {
						error: e.message,
						code: e.code
					});
					throw e;
				...
				json$1(res, e instanceof LauncherError ? 400 : 500, { error: msg });
			}
		}
	};
	const entry = {
		...
		handler: async (req, res) => {
			...
			const opts = body;
			if (typeof opts.id !== "string" || typeof opts.file !== "string" || opts.file === "") return json$1(res, 400, { error: "缺 id / file" });
			const rec = installer.get(opts.id);
			if (rec === void 0) return json$1(res, 404, { error: `未安装: ${opts.id}` });
			if (!installer.listFiles(installer.appDir(rec.slug)).includes(opts.file)) return json$1(res, 400, { error: `入口不在包内: ${opts.file}` });
			try {
				json$1(res, 200, {
					ok: true,
				...
				json$1(res, 500, { error: String(e.message ?? e) });
			}
		}
	};
	const preview = {
		...
		handler: async (req, res) => {
			...
			const opts = body;
			if (typeof opts.id !== "string" || typeof opts.dataUrl !== "string") return json$1(res, 400, { error: "缺 id / dataUrl" });
			const rec = installer.get(opts.id);
			if (rec === void 0) return json$1(res, 404, { error: `未安装: ${opts.id}` });
			const pv = dataUrlToBytes(opts.dataUrl);
			if (pv === null) return json$1(res, 400, { error: "dataUrl 非法（仅支持 data:image/*;base64）" });
			try {
	...
	const uninstall = {
		...
		handler: (req, res) => {
			...
			if (!installer.has(id)) return json$1(res, 404, { error: `未安装: ${id}` });
			installer.remove(id);
			json$1(res, 200, { ok: true });
		}
	};
	...
	const previewFile = {
		...
		handler: (req, res) => {
			const id = query(req).get("id");
			if (id === null || id === "") return json$1(res, 400, { error: "缺 id" });
	...
	const auth139 = {
		...
		handler: async (req, res) => {
			...
			const opts = body;
			if (typeof opts.authorization !== "string") return json$1(res, 400, { error: "缺 authorization" });
			const v = opts.authorization.trim();
			if (v !== "") try {
				normalize139Authorization(v);
			} catch (e) {
				return json$1(res, 422, { error: `拒绝保存，不是有效的 139 登录态：${e.message}` });
			}
			cred139.write(v);
			json$1(res, 200, {
				ok: true,
	...
	/** 登录态同步助手（Tampermonkey 脚本）：canonical /we-sync/login-sync.user.js；
	*  旧链接 /we-sync/139-helper.user.js 同样服务合并脚本，已装旧版的浏览器手动更新一次即得全量。 */
	const helperSyncHandler = (_req, res) => {
		res.statusCode = 200;
	...
	return [
		...
		{
			kind: "exact",
			path: HELPER_SYNC_URL,
			handler: helperSyncHandler
		},
		{
			kind: "exact",
			path: "/we-sync/139-helper.user.js",
			handler: helperSyncHandler
		},
		rootRoute
	];
...
function parseSync(body) {
	...
	return {
		pageId: typeof o.pageId === "string" && o.pageId !== "" ? o.pageId.slice(0, 64) : void 0,
		enabled: o.enabled,
		state: o.state,
		title: typeof o.title === "string" ? o.title.slice(0, 300) : void 0,
		color: typeof o.color === "string" && /^#?[0-9a-fA-F]{6}$/.test(o.color) ? o.color.replace("#", "").toLowerCase() : void 0
	};
}
const RESPAWN_MS = 2e3;
/** 可见页 5s 心跳一发；连丢 ~3 拍即认为该页已关闭/假死，从注册表剔除 */
const VISIBLE_TTL_MS = 15e3;
/** 无 pageId 的上报（旧版 client）共用这一个桶。 */
const LEGACY_PAGE = "legacy";
var FloaterHub = class {
	...
	sync(p) {
		...
		if (!p.enabled || p.state === "unloading") this.pages.delete(id);
		else this.pages.set(id, {
			hidden: p.state === "hidden",
			ts: this.deps.now()
		});
		this.reconcile();
	}
	...
	reconcile() {
		...
		for (const [id, info] of this.pages) {
			...
			if (now - info.ts > 15e3) this.pages.delete(id);
			else if (expiry < nextPruneAt) nextPruneAt = expiry;
		}
		this.armPrune(nextPruneAt, now);
		let anyVisible = false;
		let anyHidden = false;
		for (const info of this.pages.values()) if (info.hidden) anyHidden = true;
...
function parseEvent(line) {
	...
	if (t === "click ok") return {
		kind: "click",
		ok: true
	};
	if (t === "click miss") return {
		kind: "click",
		ok: false
	};
...
var FloaterManager = class {
	...
	constructor(opts) {
		...
		this.log = opts.log ?? (() => {});
		this.exePath = opts.exePath !== void 0 ? opts.exePath : resolveFloaterExe();
	...
	loadPos() {
		...
		try {
			const raw = JSON.parse(readFileSync(this.posFile, "utf8"));
	...
	persistPos(x, y) {
		...
		try {
			...
			writeFileSync(this.posFile, JSON.stringify({
				x,
				y
			}), "utf8");
		} catch {}
	...
	startProc() {
		...
		try {
			...
			this.child = null;
			this.log("floater: spawn 失败 " + String(e.message ?? e));
		...
		child.on("error", (err) => this.log("floater: 子进程错误 " + String(err.message ?? err)));
		child.on("exit", () => {
			this.child = null;
			this.stdoutBuf = "";
	...
	writeStdin(line) {
		...
		try {
			...
			this.log("floater: stdin 写入失败 " + String(e.message ?? e));
		}
	}
	onStdout(chunk) {
	...
	handleLine(line) {
		...
		switch (ev.kind) {
			...
			case "dismiss":
				...
				break;
			case "click":
				this.hub.onClick(ev.ok);
				break;
			case "moved":
				this.hub.onMoved(ev.x, ev.y);
				break;
			case "startup-fail": this.log("floater: 启动失败 " + ev.reason);
		}
	}
	...
	return new Promise((resolve, reject) => {
		...
		stream.on("data", (c) => {
			...
			if (size > MAX_BODY_BYTES) {
				done(() => reject(/* @__PURE__ */ new Error("请求体超过上限")));
				return;
			}
			chunks.push(c);
		});
...
function createFloaterRoutes(deps) {
	...
	return [{
		...
		path: base,
		handler: async (req, res) => {
			if (req.method === "GET" || req.method === void 0 || req.method === "") return json(res, 200, status());
			if (req.method !== "POST") return json(res, 405, { error: "method not allowed" });
			let parsed;
			try {
				parsed = JSON.parse(new TextDecoder().decode(await readBody(req)));
			} catch (e) {
...
function apply(ctx) {
	...
	*  支持两种形态：目录本身是一个壁纸（内含 project.json），或是一个集合目录（子目录各为壁纸）。 */
	...
	for (const route of createFloaterRoutes({
		hub: floater.hub,
		supported: () => floater.supported
	})) disposers.push(webServer.register(route));
		...
		handler(req, res) {
			...
			execFile("explorer.exe", ["/select," + file.replace(/\//g, "\\")], { windowsHide: true }, (err) => {
				...
				execFile("powershell.exe", [
					"-NoProfile",
					"-Command",
					"Invoke-Item -LiteralPath '" + dir.replace(/'/g, "''") + "'"
	...
	*  client 半用 @dwp/web 的 mount() 拉这些端点组装背景层。 */
	...
	if (CONFIG.workspacePulseAutoInstall && !market.store.has("workspace-pulse")) try {
		...
		market.store.upsert({
			...
			sourceUrl: "builtin:workspace-pulse",
			path: "packages/workspace-pulse.dwp",
			installedAt: (/* @__PURE__ */ new Date()).toISOString(),
			commercial: false
		});
	} catch (e) {
		console.warn("[we-sync] 内置 DWP（工作区脉搏）自动入库失败（不阻断插件）：", e.message ?? e);
	}
