...
var LauncherInstaller = class {
	...
	async download(url) {
		...
		try {
			...
			throw new LauncherError(`下载请求失败: ${e.message ?? e}`);
		}
	...
	*/
	extract7z(bytes, destDir, password) {
		const tools = [];
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					...
					let files;
					try {
						if (is7z) files = installer.extract7z(bytes, dir, password);
					...
					const msg = e instanceof LauncherError ? e.message : String(e.message ?? e);
