	...
	return [
		{
			...
			handler: async (_req, res) => {
				try {
					json$1(res, 200, await ensureCatalog());
				} catch (e) {
					json$1(res, 502, { error: String(e.message ?? e) });
				}
			}
		},
		{
			...
			path: base + "/installed",
			handler: (_req, res) => {
				json$1(res, 200, { installed: deps.market.installed() });
			}
		},
		{
			kind: "exact",
			...
			handler: async (req, res) => {
				...
				try {
					...
					return json$1(res, 502, { error: String(e.message ?? e) });
				}
				if (!entry) return json$1(res, 404, { error: `catalog 无此 id: ${id}` });
				try {
					json$1(res, 200, {
						ok: true,
					...
					if (e instanceof NeedsPurchaseError) return json$1(res, 402, {
						error: "needs-purchase",
						salesUrl: e.salesUrl,
						platform: e.platform
					});
					return json$1(res, 500, { error: String(e.message ?? e) });
				}
			}
		},
		{
		...
		},
		{
			...
			handler: async (_req, res) => {
				try {
					json$1(res, 200, { updates: deps.market.updates(await ensureCatalog()) });
				} catch (e) {
					json$1(res, 502, { error: String(e.message ?? e) });
				}
			}
...
function readZip(bytes) {
	...
	for (let i = bytes.length - 22; i >= min; i--) if (view.getUint32(i, true) === SIG_EOCD) {
		eocd = i;
		break;
	}
...
function createDwpServeRoutes(deps) {
	...
	const file = {
		...
		handler: (req, res) => {
			...
			if (!data) return json(res, 404, { error: `包内无此文件: ${name}` });
			res.statusCode = 200;
	...
	const jsonEntry = (path, defaultName) => ({
		...
		handler: (req, res) => {
			...
			if (!data) return json(res, 404, { error: `包内无 ${want}` });
			res.statusCode = 200;
			res.setHeader("Content-Type", "application/json; charset=utf-8");
