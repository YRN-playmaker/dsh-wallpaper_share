	...
	return [
		{
			...
			handler: async (req, res) => {
				try {
					json$2(res, 200, await ensureCatalog(query$2(req).get("refresh") === "1"));
				} catch (e) {
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				if (rec !== void 0 && q.get("force") !== "1" && compareVersion(next, rec.version) < 0) return json$2(res, 409, {
		...
		},
		{
			...
			handler: async (req, res) => {
				try {
					json$2(res, 200, { updates: deps.market.updates(await ensureCatalog(query$2(req).get("refresh") === "1")) });
				} catch (e) {
					json$2(res, 502, { error: String(e.message ?? e) });
				}
			}
...
//#endregion
//#region src/launcher/baiduyun.ts
/**
...
const BAIDU_DOWNLOAD_UA = "netdisk";
/** 识别百度网盘分享链接：/s/1<id>（可选 ?pwd=）或 /share/init?surl=<id>。非百度链接返回 null。 */
function parseBaiduShareUrl(url) {
	const text = url.trim();
...
var BaiduError = class extends LauncherError {
	code;
	constructor(code, msg) {
		super(msg);
...
function errnoToError(errno, where) {
...
*/
function normalizeBaiduCookie(raw) {
	let s = raw.trim();
	...
	const pairs = s.includes("=") ? s.split(";").map((p) => p.trim()).filter((p) => p !== "") : [`BDUSS=${s.replace(/\s+/g, "")}`];
	const kept = [];
	for (const p of pairs) {
		const eq = p.indexOf("=");
		if (eq <= 0) continue;
	...
	if (!kept.some((p) => p.startsWith("BDUSS="))) throw new BaiduError("share_auth_required", "不是有效的百度网盘登录态（需要 BDUSS）。任选其一：① 装「百度登录态同步助手」油猴脚本，登录 pan.baidu.com 后自动同步（面板可一键打开）② F12 → 网络 → 刷新 → 点任一 pan.baidu.com 请求 → 请求标头 → 复制整行 Cookie: 粘贴到这里 ③ F12 → 应用 → Cookie → pan.baidu.com → 复制 BDUSS 的值直接粘贴");
	return kept.join("; ");
}
/** 从登录态串里挖 BDUSS 值（掩码展示用）。 */
function bdussOf(cookie) {
	const m = /BDUSS=([A-Za-z0-9~_-]+)/.exec(normalizeBaiduCookie(cookie));
	return m !== null ? m[1] : "";
}
function baiduFileCredStore(path) {
	return {
		read() {
			try {
				const j = JSON.parse(readFileSync(path, "utf8"));
...
var BaiduClient = class {
	...
	async listRoot(surl, pwd, cookie) {
		...
		return {
			needVerify: false,
			files: (Array.isArray(j.list) ? j.list : []).map((e) => ({
				fsId: String(e.fs_id ?? ""),
	...
	async verifyPwd(surl, pwd, cookie) {
		const qs = new URLSearchParams({
			surl,
			t: String(Date.now()),
			channel: "chunlei",
	...
	*/
	async dlinkViaContentPage(surl, fsId, cookie) {
		const res = await this.fetchFn(`${HOST}/s/1${surl}`, { headers: this.baseHeaders(cookie) });
		const html = await res.text();
		if (!res.ok) throw new BaiduError("share_api_error", `百度内容页拉取失败（HTTP ${res.status}），无法解析下载签名`);
		...
		if (sign === "" || timestamp === "" || shareid === "") throw new BaiduError("share_api_error", "内容页缺少下载签名（sign/timestamp/shareid）：登录态可能失效或页面结构变化");
		const qs = new URLSearchParams({
			sign,
			timestamp,
			bdstoken: "",
		...
		const text = await (await this.fetchFn(`${HOST}/api/download?${qs.toString()}`, {
			method: "POST",
			headers: {
				...this.baseHeaders(cookie),
				"Content-Type": "application/x-www-form-urlencoded",
		...
		const err = errnoToError(typeof j.errno === "number" ? j.errno : Number(j.errno ?? -1), "download");
		if (err !== null) throw err;
	...
	async resolve(shareUrl, passcode) {
		...
		const pwd = passcode !== void 0 && passcode !== "" ? passcode : ref.pwd;
		const auth = this.getAuth();
		if (auth === "") throw new BaiduError("share_auth_required", "百度网盘下载需要登录态：请在面板「百度网盘登录态」里粘贴 BDUSS Cookie");
		let cookie;
		try {
			cookie = normalizeBaiduCookie(auth);
		} catch (e) {
			throw new BaiduError("share_auth_required", `已配置的百度登录态无效，请重新粘贴（详情：${e.message}）`);
		}
		let files;
		let bdclnd = "";
		let listed = await this.listRoot(ref.surl, pwd, cookie);
		if (listed.needVerify) {
			if (pwd === void 0 || pwd === "") throw new BaiduError("share_passcode_required", "该百度网盘分享需要提取码：请在提取码框填入后重试");
			bdclnd = await this.verifyPwd(ref.surl, pwd, cookie);
			const withClnd = bdclnd !== "" ? `${cookie}; BDCLND=${bdclnd}` : cookie;
			listed = await this.listRoot(ref.surl, void 0, withClnd);
			if (listed.needVerify) throw new BaiduError("share_passcode_wrong", "提取码验证通过但列表仍要求验证（可能登录态失效）");
		}
		files = listed.files;
		if (files.some((f) => f.isDir)) throw new BaiduError("share_not_file", `百度分享根目录含文件夹（暂不支持文件夹分享）: ${files.filter((f) => f.isDir).map((f) => f.name).slice(0, 5).join(", ")}`);
		if (files.length === 0) throw new BaiduError("share_not_file", "百度分享内容为空");
		if (files.length > 1) throw new BaiduError("share_not_file", `百度分享根目录不是单个文件（暂不支持多文件分享）: ${files.map((f) => f.name).slice(0, 5).join(", ")}`);
		const file = files[0];
		const downloadUrl = file.dlink !== void 0 && file.dlink !== "" ? file.dlink : await this.dlinkViaContentPage(ref.surl, file.fsId, cookie);
...
function createLauncherRoutes(deps) {
	...
	const credBaidu = deps.credBaidu ?? {
		read: () => "",
		write: () => {}
	};
	const baidu = deps.baidu ?? new BaiduClient({ getAuth: () => credBaidu.read() });
	...
	const install = {
		...
		handler: async (req, res) => {
			...
			try {
				...
				else if (parseBaiduShareUrl(url) !== null) try {
					const meta = await baidu.resolve(url, passcode);
					url = meta.downloadUrl;
					shareFileName = meta.fileName;
					dlHeaders = meta.headers;
				} catch (e) {
					if (e instanceof BaiduError) return json(res, 422, {
						error: e.message,
						code: e.code
					});
					throw e;
				}
				const dl = await installer.download(url, dlHeaders !== void 0 ? { headers: dlHeaders } : void 0);
				...
				try {
					...
					if (e instanceof CryptZipError) return json(res, 422, {
						error: e.message,
						code: e.code
					});
					throw e;
	...
	const entry = {
		...
		handler: async (req, res) => {
			...
			if (!installer.listFiles(installer.appDir(rec.slug)).includes(opts.file)) return json(res, 400, { error: `入口不在包内: ${opts.file}` });
			try {
				json(res, 200, {
					ok: true,
	...
	*  已装旧版脚本的浏览器手动更新一次即得全量。 */
	...
	const helperLegacy139 = {
		kind: "exact",
		path: "/we-sync/139-helper.user.js",
		handler: helperSyncHandler
	};
	const helperLegacyBaidu = {
		kind: "exact",
		path: "/we-sync/baidu-helper.user.js",
		handler: helperSyncHandler
	};
	return [
		installed,
		...
		auth139,
		{
			...
			handler: async (req, res) => {
				...
				if (v !== "") try {
					normalizeBaiduCookie(v);
				} catch (e) {
					return json(res, 422, { error: `拒绝保存，不是有效的百度网盘登录态：${e.message}` });
				}
				credBaidu.write(v);
				json(res, 200, {
					ok: true,
