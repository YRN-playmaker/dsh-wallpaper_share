...
function normalizeBaiduCookie(raw) {
	...
	const pairs = s.includes("=") ? s.split(";").map((p) => p.trim()).filter((p) => p !== "") : [`BDUSS=${s}`];
	const kept = [];
	for (const p of pairs) {
		const eq = p.indexOf("=");
		if (eq <= 0) continue;
	...
	if (!kept.some((p) => p.startsWith("BDUSS="))) throw new BaiduError("share_auth_required", "不是有效的百度网盘登录态（需要 BDUSS Cookie：F12 → 应用/网络 → 复制 BDUSS，建议连 STOKEN 一起）");
	return kept.join("; ");
}
