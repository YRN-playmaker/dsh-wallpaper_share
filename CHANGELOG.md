...
- **专注「清晰窗」透镜跟随视线**：面板新增「眼动追踪」开关（+ 可选「校准视线」按钮）。开启后**惰性**从 CDN 加载 [WebGazer.js](https://webgazer.cs.brown.edu)（GPL-3.0，与本项目许可兼容；内含 MediaPipe FaceMesh，首次约下载 ~12MB，**不进基础包**），用摄像头推断屏幕注视点。透镜语义改为**视线处保持清晰圆、四周柔焦**（让注视到的文字清晰可读）：壁纸全局模糊在透镜激活时置 0，改由透镜层用反向 mask 的 `backdrop-filter` 只在清晰圆之外施加模糊。
  - **入场动画**：进入专注时先全屏模糊，清晰圆半径在 ~1.4s 内由 0 缓出"汇聚"到视线处（`--wesync-lens-r` 每帧驱动）。
  - **免校准（隐式自学习）**：默认无需任何校准，WebGazer 在 `begin()` 期间持续从鼠标移动 / 点击采样自校准，样本经 localforage 存 IndexedDB 跨会话复用、越用越准；「校准视线」9 点序列降级为可选的精度加速。
  - **抗抖动**：`pumpLens` 对注视点做 EMA 平滑（`GAZE_SMOOTH`），配合 WebGazer 自带 Kalman 滤波消除"乱跳"；`getGaze()` 带 1.2s 时效，无脸 / 离开座位自动回落鼠标。
  ...
  - 可调参数：`FOCUS_LENS_RADIUS`（清晰圆大小）、`LENS_SURROUND_BLUR`（圆外模糊）、`LENS_ENTER_MS`（入场时长）、`GAZE_SMOOTH`（平滑强度）。
  - 边界：需摄像头 + 联网加载模型；开启期间摄像头常开（opt-in，可随时关）；零样本初期注视点可能有偏移，随鼠标使用收敛。
