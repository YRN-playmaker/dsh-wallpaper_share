...
- **双页一体滚动**：面板改为「设置 ⇄ 壁纸库」两页**纵向叠放，一个原生滚动搞定**（不拦截 wheel，与浏览器/宿主滚动手感完全一致）；页界 **scroll-snap 吸附**防误触——页内任意位置停下都被拉回整页对齐，滚过页底半屏才吸附翻到下一页，`scroll-snap-stop: always` 保证一滚只翻一页；滚动条隐藏但滚轮/触摸板/拖动全部原生可用。右侧页签 **scrollspy 跟随当前页**（黄色高亮发光，贴面板右缘），点击原生平滑滚到对应页；壁纸库数据首次滚入时懒加载。原「浏览壁纸」展开按钮取消（滚动替代），壁纸读取位置（地址管理）归入设置页。
