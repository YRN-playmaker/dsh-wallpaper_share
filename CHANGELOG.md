...
- **注视点透镜（眼动 / 专注）**：面板新增「眼动追踪」开关（+ 可选「校准视线」）。开启后**惰性**从 CDN 加载 [WebGazer.js](https://webgazer.cs.brown.edu)（GPL-3.0，与本项目许可兼容；内含 MediaPipe FaceMesh，首次约下载 ~12MB，**不进基础包**），用摄像头推断屏幕注视点。壁纸全局模糊在透镜激活时置 0，模糊全部由透镜层 `backdrop-filter` 承担。
  - **圆心清晰 / 模糊可切换**：面板「圆心清晰 / 圆心模糊」按钮切换两种形态——清晰=阅读窗（圆心透明、圆外模糊，让注视处文字可读）；模糊=圆盘（圆心不透明、圆外清晰）。mask 方向随之反转。
  ...
  - **抗抖动**：注视点用**死区 + EMA**（`GAZE_DEADZONE=12`、`GAZE_SMOOTH=0.08`）——小于死区的高频抖动直接忽略冻结、只有大幅移动才缓动跟随（实测静态漂移比纯 EMA 再降 ~35%）。此前试过 One Euro 自适应滤波，但它会主动跟随眼跳、反而更"跳"，故回退到求稳的死区 + EMA。
  - **文字吸附**（`gazeSnapText`，默认关）：`elementFromPoint` 命中注视处元素并向上找最近文本块磁吸；因当前"磁吸到块中心"的表现待与需求对齐，暂默认关闭。
  ...
  - 可调参数：`FOCUS_LENS_RADIUS`（圆大小）、`LENS_BLUR`（模糊强度）、`LENS_ENTER_MS`（入场时长）、`GAZE_DEADZONE`（死区，越大越稳）、`GAZE_SMOOTH`（EMA，越小越稳）、`SNAP_PULL`（吸附强度）。
