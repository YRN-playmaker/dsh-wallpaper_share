...
- **双页翻滚**：面板改为「设置 ⇄ 壁纸库」两页，滚轮**高阻尼蓄力翻页**——拦截原生 wheel（`passive: false`），`deltaY` 动量累加突破阈值 600 才切页（常规轻滑不触发）；未突破时页面随累加值产生约 7% 的**阻力拉扯**跟随；停止滚动 250ms 后动量清零、页面平滑弹回原位。翻页带方向性进出场动画（下滚进壁纸库 / 上滚回设置），内部滚动容器（壁纸网格、市场列表）仍原生滚动优先，不被翻页劫持。原「浏览壁纸」展开按钮取消（翻页替代），壁纸读取位置（地址管理）归入设置页。
- **右侧醒目页签**：GPT 式竖排页签常驻面板右缘——当前页高亮黄色描边发光，蓄力时目标页签内**进度条随累加比例充能**（超时衰减自动清零），页签可直接点击切页（兜底入口，触控板轻滑用户也能用）。
